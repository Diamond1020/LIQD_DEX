...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  liqdPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const LiqdPerBlock = liqdPerBlock ? new BigNumber(liqdPerBlock).div(BIG_TEN.pow(9)) : BIG_ZERO
  ...
  return {
    ...
    liqdPerBlock: new BigNumber(LiqdPerBlock).toJSON(),   
  }
}
